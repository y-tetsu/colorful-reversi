...
// Player
let game = new Game(TEST_BOARD1, ORDER, FLIPPERS);
let player = new Player('unknown');
assertEqual(player.actMove(game), [], 'Player 1');

game = new Game(TEST_BOARD1, ORDER, FLIPPERS);
...
// getMoveByRandom
for (let i=0; i<10; i++) {
  let move = getMoveByRandom(new Game(TEST_BOARD1, ORDER, FLIPPERS));
  assertEqual(getLegalMoves(B, TEST_BOARD1).includes(move), true, 'getMoveByRandom ' + (i + 1));
}

// getMoveByMonteCarloSearch
function testGetMoveByMonteCarloSearch(boards, orders, flipperss, schedules, expecteds) {
  let i = 0;
  for (board of boards) {
    const game = new Game(board, orders[i], flipperss[i]);
...
testGetMoveByMonteCarloSearch(boards, orders, flipperss, schedules, expecteds);
...
function testPlayoutValue(indexs, boards, orders, expecteds) {
  let i = 0;
  for (board of boards) {
    assertEqual(getPlayoutValue(indexs[i], board, orders[i], RANDOM_FLIPPERS), expecteds[i], 'getPlayoutValue ' + (i + 1));
    i++;
  }
}
